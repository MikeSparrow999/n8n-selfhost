...
| backup.sh: rclone **403 AccessDenied / 501** intermittently | `rclone --bind <vps-ipv4> lsd r2:n8n-backups` works but plain `rclone lsd` doesn't | The R2 token is IP-filtered to the IPv4; rclone went out over IPv6. Ensure `RCLONE_BIND_ADDR=<vps-ipv4>` is in `.env` (backup.sh exports it). If the VPS IP ever changes, update BOTH the token's IP filter and `.env` |
