...
| backup.sh: rclone logs **501 NotImplemented** on attempt 1, succeeds on attempt 2 | `backups/backup.log` | Known R2 + rclone quirk (checksum header on first PUT). Harmless — retries succeed. If it ever fails all 3 attempts, try `rclone config update r2 use_multipart_uploads=false` |
